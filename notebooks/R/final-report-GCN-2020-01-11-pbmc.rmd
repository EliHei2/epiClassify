...
unique(train_y$class_) %>% map(~cell_type_graph_imp(imp_MLP, .x, paste0(.x, '_MLP')))
unique(train_y$class_) %>% map(~cell_type_graph_imp(imp_GraphSAGE, .x, paste0(.x, '_GraphSAGE')))
```
...
cell_type_graph_layer <- function(type, title){
    ...
    igraph::V(graph1)$color  = rgb(floor(255*(grp^(1/2))/max(grp^(2))), floor(255*(grp^(2))/max(grp^(2))), 0, maxColorValue=255, alpha=255)
